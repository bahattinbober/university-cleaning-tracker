...
class AdminAllCleaningsScreen extends StatefulWidget {
  ...
  @override
  State<AdminAllCleaningsScreen> createState() =>
      _AdminAllCleaningsScreenState();
}
  ...
  Future<void> _fetchAllLogs() async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('http://192.168.1.27:4000/api/cleaning'),
                  ...
                  : RefreshIndicator(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final roomName =
                              log['room_name']?.toString() ?? 'Oda';
                          final cleanerName =
                              log['user_name']?.toString() ?? 'Personel';
                          final status = StatusTranslator.cleaningStatus(
                            log['status']?.toString(),
                          );
                          final notes = log['notes']?.toString() ?? '';
                          final cleanedAt = _formatDate(
                            log['cleaned_at']?.toString(),
                          );
